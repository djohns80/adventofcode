...
graph, flows = parse(open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'sample'), 'r', encoding='utf-8').read())
pressure, path, opened, debug = find(graph, flows, "AA")
# for a in debug:
#     print(a)
assert pressure == 1651

graph, flows = parse(open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'input'), 'r', encoding='utf-8').read())
pressure, path, opened, debug = find(graph, flows, "AA")
print(pressure)
