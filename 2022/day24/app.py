...
#data = open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'sample'), 'r', encoding='utf-8').read().strip().splitlines()
data = open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'input'), 'r', encoding='utf-8').read().strip().splitlines()
depth = len(data)-2
width = len(data[0])-2
start = (1,0)
end = (width, depth+1)
walls = set([(x,y) for x in (0,width+1) for y in range(depth+2)] + [(1,-1),(width, depth+2)] + [(x,y) for y in (0,depth+1) for x in range(width+2) if ((x,y) != start and (x,y) != end)])
...
print(p1 := bfs(start, end, 0))
print(bfs(start, end, bfs(end, start, p1)))
