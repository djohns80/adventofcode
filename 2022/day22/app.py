...
x = open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'sample'), 'r', encoding='utf-8').read()
grid, ins = x.split("\n\n")
...
#    tiles = [int(v) if v[0].isdigit() else v for v in re.findall(r'\d+|L|R', lines.pop())]
#    map = {(x + 1, y + 1): c for y, l in enumerate(lines) for x, c in enumerate(l) if c != ' '}
#    pos = (1, 1)
#    for x, p in enumerate(lines[0]):
#        if p == '.':
#            pos = (x + 1, 1)
#            break
#    facing = 'R'
