...
def f(lines):
    ...
    def helper(hallway, rooms):
        ...
        for i, square in enumerate(hallway):  # Move from the hallway into a room.
            ...
            helper_result = helper(hallway[:i] + (None,) + hallway[i + 1:], rooms[:dest] + (new_room,) + rooms[dest + 1:])
...
# part 1 #
##########
    data = [l.replace('#','').strip() for l in input if l.replace('#','').replace('.','').strip() != '']
    data = tuple(zip(*[[c for c in l] for l in data]))
    print(f(data))

##########
# part 2 #
##########
    input2 = [*input[:3], *['  #D#C#B#A#', '  #D#B#A#C#'], *input[3:]]
    data2 = [l.replace('#','').strip() for l in input2 if l.replace('#','').replace('.','').strip() != '']
    data2 = tuple(zip(*[[c for c in l] for l in data2]))
    print(f(data2))
